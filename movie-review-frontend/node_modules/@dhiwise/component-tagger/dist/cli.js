 ...
 */
function createBrowserScript(options) {
 ...
 */
function injectHtmlComponentTagger(options = {}) {
    const pluginOptions = {
        extensions: ['.html', '.htm'],
        verbose: false,
    ...
    const outputDir = path__namespace.resolve(cwd, options.outputDir || 'public');
    ...
    const outputFile = path__namespace.join(outputDir, scriptName);
    verboseLog(`Writing script to: ${outputFile}`, pluginOptions);
    fs__namespace.writeFileSync(outputFile, browserScriptContent);
    stats.scriptPath = outputFile;
    const extensions = pluginOptions.extensions;
    const extensionPattern = `**/*{${extensions.join(',')}}`;
    const htmlFiles = glob.sync(extensionPattern, {
        cwd,
        ignore: [
            'node_modules/**',
            ...pluginOptions.excludeDirectories.map((dir) => `${dir}/**`),
        ],
    });
    stats.totalFiles = htmlFiles.length;
    verboseLog(`Found ${htmlFiles.length} HTML files to process`, pluginOptions);
    htmlFiles.forEach((htmlFile) => {
        const filePath = path__namespace.join(cwd, htmlFile);
        let content = fs__namespace.readFileSync(filePath, 'utf8');
        stats.processedFiles++;
        const injectorScriptPath = (() => {
            const relativePath = path__namespace
                .relative(path__namespace.dirname(filePath), outputFile)
                .replace(/\\/g, '/');
...
try {
    ...
    verboseLog(`  - Files with script injected: ${result.injectedFiles}`, options);
